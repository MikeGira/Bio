...
function classify(result) {
  ...
  // Vercel bot mitigation can 403 datacenter IPs; that is inconclusive, not an outage.
...
async function main() {
  ...
  const home = await probeWithRetry(SITE);
  ...
  if (homeState === 'ok') {
    ...
    if (missing.length > 0) {
      failures.push(`**Security headers missing:** ${missing.join(', ')} — deploy/config regression vs vercel.json.`);
    } else {
      console.log('Security headers: all present');
    }
  }

  const api = await probe(`${SITE}/api/db`);
